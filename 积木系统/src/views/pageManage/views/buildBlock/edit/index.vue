  ...
  <div class="page-box" >
    ...
    <div class="page-box-right">
      <div class="page-box-right-top">
          <el-button type="primary" @click="save"  :disabled="buildBlock.titleData.status === '1'">保存为草稿</el-button>
          <el-button type="primary" @click="clickPraViews">预览</el-button>
...
import { getBlockById } from '@/api/pageManage/index'
export default {
  components: {
    leftSide,
    mobileView,
  ...
  mounted() {
    ...
    if (specialId) {
      ...
      getBlockById(specialId).then(res => {
        const data = res.data
        const jmData = JSON.parse(JSON.stringify(res.data.pages.content || []))
        delete data.pages
        this.$store.dispatch(this.toolType + '/SET_TITLE_DATA', data).then((res) => {
          this.$store.dispatch(this.toolType + '/SET_ACTIVECOMPONET', { name: 'title', uuid: '' })
        })
        this.$store.dispatch(this.toolType + '/INIT_JM_DATA', jmData)
      }).catch(res => {
        this.$store.dispatch(this.toolType + '/SET_TITLE_DATA', { editor: createName }).then(res => {
          this.$store.dispatch(this.toolType + '/SET_ACTIVECOMPONET', { name: 'title', uuid: '' })
        })
        this.$store.dispatch(this.toolType + '/INIT_JM_DATA', [])
      })
    }
  },
  methods: {
