  ...
  <div class="nav-box" >
    <div class="nav-box-content-content" ref="navBox">
       <div class="nav-box-content-box"  :style="[{'width': width}, navStyle ]">
         <span class="nav-box-item nav-box-item-pre" v-for="item in label" :key="item.uuid" :style="navActiveUuid == item.modelId ? navActiveStyle : ''" :id="'nav-pre-' + item.modelId"> {{item.name}}</span>
       </div>
...
export default {
    ...
    label() {
      ...
      this.jmData.forEach(item => {
        if (item.businessData.modelName && item.businessData.modelId) {
          label.push({ uuid: item.uuid, name: item.businessData.modelName, modelId: item.businessData.modelId })
        }
      })
      return label || []
    }
  ...
  methods: {
    ...
    getWidth() {
      this.$nextTick(() => {
        const doms = document.getElementsByClassName('nav-box-item-pre')
        if (!doms || doms.length < 1) {
          ...
          this.width = width > 373 ? width + 'px' : '373px'
        }
      })
    },
    ...
    scrollNavLeft(uuid) {
      this.$nextTick(() => {
        ...
        const dom = document.getElementById('nav-pre-' + uuid)
        if (!dom) return false
