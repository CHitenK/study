  ...
  <div>
    ...
    >
      <el-row :gutter="10" class="fs-14 clr-II detail-row">
        ...
        <el-col :span="12" class="mr-b-10">
          预付总额：{{(detail.currency || '') + ' ' +  (detail.prepaymentAmount || 0)}}
        </el-col>
        ...
        <el-col :span="24" class="mr-t-10">
          ...
          <JFX-table :tableData.sync="tableData"
            ...
            <template slot="paymentDate" slot-scope="{row}">
              ...
              />
              <span v-else>{{row.paymentDateStr ? row.paymentDateStr.slice(0, 10) : ''}}</span>
            </template>
            <template slot="paymentAmount" slot-scope="{row}">
              <JFX-Input v-if="row.edit" v-model.trim="row.paymentAmount" :precision="2" :min="0" style="width: 100%" />
...
export default {
  ...
  data() {
    return {
      ...
      // 表格列数据
      tableColumn: [
        { label: '本次付款日期', slotTo: 'paymentDate', minWidth: '150', align: 'center', hide: true },
        { label: '本次付款金额', slotTo: 'paymentAmount', minWidth: '150', align: 'center', hide: true },
        { label: '付款流水号', slotTo: 'serialNo', minWidth: '150', align: 'center', hide: true },
        { label: '操作人', prop: 'creatorName', width: '120', align: 'center', hide: true },
  ...
  methods: {
    async init() {
      try {
        ...
        this.tableData.list = list ? list.map(item => ({ ...item, paymentDateStr: item.paymentDate })) : []
    ...
    async comfirm() {
      ...
      try {
        ...
        const { paymentAmount, paymentDateStr, serialNo } = this.tableData.list.find(item => item.edit === true)
