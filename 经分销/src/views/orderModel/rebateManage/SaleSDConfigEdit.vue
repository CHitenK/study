<template>
  ...
  <div class="page-bx bgc-w sales-add-bx">
      ...
      <el-row :gutter="10">
        ...
        <el-col :xs="24" :sm="12" :md="12" :lg="8" :xl="6">
          <el-form-item label="平台备注：" prop="remark">
            <el-input v-model="ruleForm.remark" placeholder="请输入" clearable />
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    ...
    <el-row class="mr-b-10">
      ...
      <el-button type="primary" @click="showModal('addSaleSDConfig')">增加</el-button>
    </el-row>
    <!-- 表格 -->
...
import { saveSdSaleConfig, sdSaleConfigDetail } from '@a/rebateManage/index'
export default {
  mixins: [commomMix],
  components: {
  ...
  data() {
    return {
      ...
      ruleForm: {
        ...
        customerId: '',
        customerName: '',
        effectiveDateStr: this.$formatDate(Date.now(), 'yyyy-MM-dd') + ' 00:00:00',
        expirationDateStr: this.$formatDate(Date.now(), 'yyyy-MM-dd') + ' 23:59:59',
        remark: '',
        operate: '1'
      },
      // 表单校验规则
      rules: {
        ...
        effectiveDateStr: { required: true, message: '请选择生效时间', trigger: 'change' },
        expirationDateStr: { required: true, message: '请选择失效时间', trigger: 'change' },
  ...
  methods: {
    ...
    async handleSubmit(type) {
      this.$refs['ruleForm'].validate(async(valid) => {
        if (valid) {
          ...
          const { effectiveDateStr, expirationDateStr, merchantId, buId, customerId } = this.ruleForm
          ...
          this.tableData.list.forEach(item => {
            const { sdTypeId, sdTypeName, categoryId, categoryName, superiorParentBrandId, superiorParentBrandName, ratio, brandParentName, isAllBrandParent, brandParentId } = item
            itemList.push({ sdTypeId, sdTypeName, categoryId, categoryName, superiorParentBrandId, superiorParentBrandName, ratio, brandParentName, isAllBrandParent, brandParentId })
          })
          const opt = {
            ...this.ruleForm,
            itemList: itemList,
    ...
    comfirm(list) {
      ...
      // 校验
      list.forEach(item => {
        const { sdTypeId, categoryId, brandParentId, isAllBrandParent, superiorParentBrandName, brandParentName, superiorParentBrandId } = item
        let target = null
        if (isAllBrandParent === '1') { // 全子品牌
          target = this.tableData.list.find(gtem => +sdTypeId === +gtem.sdTypeId && +categoryId === +gtem.categoryId && +superiorParentBrandId === +gtem.superiorParentBrandId && +isAllBrandParent === +gtem.isAllBrandParent)
        } else {
          target = this.tableData.list.find(gtem => +sdTypeId === +gtem.sdTypeId && +categoryId === +gtem.categoryId && +superiorParentBrandId === +gtem.superiorParentBrandId && +brandParentId === +gtem.brandParentId)
        }
        if (target) {
          flag = false
