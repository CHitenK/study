...
import { checkDepotMerchantRel, getDepotInfo, getPurchasePriceManage, saveOrUpdateTempOrder, modifyPurchaseOrder, getPurchaseOrderDetails } from '@a/purchaseManage/index'
...
export default {
  ...
  methods: {
    ...
    async changeDepotId(type) {
      ...
      if (type !== 'init') { // 不是初始时,清空
        this.tableData.list = []
        this.ruleForm.tallyingUnit = '' // 当入仓仓库为海外仓，可填；其他场景下不可选填
      }
    },
    // 选中供应商
