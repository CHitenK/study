...
<script>
import { getPurchaseOrderDetails, purchaseDelivery } from '@a/purchaseManage/index'
    ...
    async init() {
      ...
      try {
        ...
        res.data.itemList.forEach(item => {
          const { num, unInwarehouseNum } = item
          item.oldNum = num
          item.expireNum = 0
          item.wornNum = 0
          item.normalNum = unInwarehouseNum || 0
        })
    ...
    async save() {
      ...
      for (let i = 0; i < this.tableData.list.length; i++) {
        ...
        if (parseInt(oldNum) < parseInt(num)) {
          this.$errorMsg('商品货号：' + goodsNo + '入库数量不能超过采购数量')
          flag = false
          break
        }
        if (parseInt(oldNum) > parseInt(num)) {
      ...
      if (msgGoodsNo.length > 0) {
        this.$confirm(msgGoodsNo + '货号入库总量不等于采购量，请确认是否提交', '提示', {
