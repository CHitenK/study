<template>
  ...
  <div class="page-bx bgc-w">
    ...
    <el-row class="mr-b-10 mr-t-20">
      <el-button id="sale-puton-btn" type="primary" @click="allUp">整批上架</el-button>
    </el-row>
    <JFX-table :tableData.sync="tableData" @selection-change="selectionChange" @change="getList" >
      <el-table-column type="selection" align="center" width="55" fixed="left" />
      <el-table-column prop="inGoodsNo" align="center" min-width="120" label="商品货号" fixed="left" />
      <el-table-column prop="inGoodsName" align="center" min-width="120" show-overflow-tooltip label="商品名称" />
      <el-table-column prop="barcode" align="center" min-width="150" label="条形码" />
      <el-table-column prop="tallyingUnit" align="center" min-width="120" label="海外仓理货单位" />
      <el-table-column prop="inReturnNum" align="center" min-width="120" label="计划调入好品量" />
      <el-table-column prop="inBadGoodsNum" align="center" min-width="120" label="计划调入坏品量" />
      <el-table-column align="center" width="110">
        <template slot="header">
          <span class="need">入库好品量</span>
        </template>
        <template slot-scope="{row}">
          <el-input-number v-model.trim="row.returnNum" :precision="0" :controls="false" :min="0" style="width: 100%;"></el-input-number>
        </template>
      </el-table-column>
      <el-table-column align="center" width="110">
        <template slot="header">
        ...
        <template slot-scope="{row}">
          <el-input-number v-model.trim="row.badGoodsNum" :precision="0" :controls="false" :min="0" style="width: 100%;"></el-input-number>
        </template>
      </el-table-column>
      <el-table-column align="center" width="150" label="批次号">
        <template slot-scope="{row}">
          <el-input v-model.trim="row.returnBatchNo" placeholder="请输入" clearable />
        </template>
      </el-table-column>
...
import { toSaleReturnInPage, validInDepotDate, saveSaleReturnIdepot } from '@a/salesReturnManage/index'
  ...
  methods: {
    // 拉取表格数据
      ...
      try {
        ...
        const { data, data: { itemList } } = await toSaleReturnInPage({id: this.$route.query.id})
    ...
    async handleSubmit() {
      ...
      if (!jj) return false
      // 校验入库时间
      const resdata = await validInDepotDate({ returnInDate: this.ruleForm.returnInDate, id: this.saleReturnOrder.id })
      ...
      if (jj.goodsNos.length > 0) {
        const tips = '货号: ' + jj.goodsNos.join(',') + ' 上架总量不等于计划调入量，请确认是否提交'
    ...
    handleTableData() {
      ...
      for (let i = 0; i < this.tableData.list.length; i++) {
        const { inGoodsId, outGoodsId, inGoodsNo, tallyingUnit, inReturnNum, returnBatchNo = '', inBadGoodsNum, returnNum, badGoodsNum, productionDate = '', overdueDate = '' } = this.tableData.list[i]
        ...
        goodsList.push({ inGoodsId, outGoodsId, inGoodsNo, tallyingUnit, returnNum, badGoodsNum, returnBatchNo: returnBatchNo || '', productionDate: productionDate || '', overdueDate: overdueDate || '' })
      }
