<template>
  <div class="page-bx bgc-w all-order-edit-bx">
    ...
    <el-form class="mr-t-20" :model="ruleForm" ref="ruleForm" :rules="rules" >
      <el-row  :gutter="10" class="fs-14 clr-II" v-if="ruleForm.code">
        <el-col :xs="24"  :sm="12" :md="12" :lg="6" :xl="6" >
          <el-form-item label="调拨订单号：" prop="code" >
            <el-input v-model.trim="ruleForm.code" clearable disabled></el-input>
          </el-form-item>
         </el-col>
      </el-row>
      <el-row  :gutter="10" class="fs-14 clr-II">
        ...
        <el-col :xs="24"  :sm="12" :md="12" :lg="6" :xl="6" >
          <el-form-item label="调出仓库：" prop="outDepotId" >
            <el-select v-model="ruleForm.outDepotId" placeholder="请选择" filterable clearable :data-list="getSelectBeanByMerchantRel('outDepotIdList', { type: 'a,b,c,e'})" @change="outDepotIdChange">
    ...
    <div>
      ...
      >
      <div class="import-bx">
        <importfile v-if="selfDown" @downup="downup" :url="importTransferGoodsUrl" :filterData="filterData1" :selfDown="true" :templateId="'2333'" @successUpload="successUpload" downText="商品导出"></importfile>
        <!-- 导入装箱明细资料 -->
        <importfile v-else :url="importPackingDetailsUrl" :filterData="{ id: detail.id }" :selfDown="selfDown" :templateId="157"></importfile>
      </div>
      </JFX-Dialog>
    </div>
    <!-- 导入 end -->
...
import { getTransferOrderDetail, getMustParameter, importTransferGoodsUrl, exportItemsUrl, saveTempTransferOrder, updateTransferOrder, importPackingDetailsUrl } from '@a/transferManage/index'
...
export default {
  ...
  methods: {
    async init() {
      ...
      // 编辑
      this.breadcrumb[1].meta.title = '编辑调拨订单'
      ...
      if (depotType === 'outDepotId') {
        ...
        if (res.data) {
          ...
          res.data.forEach(item => {
            arr.push({
              ...item,
              grossWeightNew: item.grossWeightSum || item.grossWeight || 0,
              netWeightNew: item.netWeightSum || item.netWeight || 0,
              seq: '',
    ...
    setTransferNum(key, index) {
      ...
      if (this.outDepotType === 'c' || this.inDepotType === 'c') {
        if (tallyingUnit === '02') {
          let { transferNum, inTransferNum } = this.tableData.list[index]
      ...
      let { inTransferNum, grossWeight, netWeight } = this.tableData.list[index]
            ...
            if (res.data) {
              ...
              res.data.forEach(item => {
                ...
                arr.push({
                  ...importData,
                  ...item,
                  grossWeightNew: importData.grossWeightSum || importData.grossWeight || 0,
                  netWeightNew: importData.netWeightSum || importData.netWeight || 0,
                  outUnit: this.outDepotType === 'c' ? this.ruleForm.tallyingUnit : '02',
          ...
          if (sepData) {
            this.$confirm('存在排序序号为空的商品，是否按现有排序提交？', {
              ...
              try {
                ...
                const res4 = await updateTransferOrder({ json: JSON.stringify(json) })
